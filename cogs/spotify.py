...
class Spotify(commands.Cog):
    ...
    async def recommendations(self, ctx, *, args=None):
        ...
        if not access_token:
            ...
            if args == None:
                ...
                seed_artists.append(result['items'][0]['id'])
                await self.send_recommendations(ctx, ','.join(seed_tracks), ','.join(seed_artists), access_token)
            elif args == 'np':
    ...
    async def send_recommendations(self, ctx, seed_tracks, seed_artists, access_token):
